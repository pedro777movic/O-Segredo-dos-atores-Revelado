export function PersuasionSection() {
  return (
    <section id="persuasion" className="py-8">
      <div className="mx-auto max-w-3xl space-y-4 text-center">
        <h2 className="font-headline text-2xl font-bold leading-tight text-foreground md:text-4xl">
          Você já brochou? Já sentiu vergonha de não dar conta do recado? Descubra como os atores profissionais mantêm{' '}
          <span className="text-primary">
            o p** duro e a retardação ejaculatória mesmo sob pressão
          </span>{' '}
          e como aplicar os mesmos métodos nas suas relações.
        </h2>
        <p className="text-xl text-muted-foreground md:text-2xl">
          Com este app-book em mãos, você terá o mesmo controle, resistência e confiança dos atores de maior performance da indústria. Surpreenda qualquer parceira com um nível de entrega que ela jamais esquecerá.
        </p>
      </div>
    </section>
  );
}
